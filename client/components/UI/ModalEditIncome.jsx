...
const ModalEditIncome = ({ incomeObject, onClose, onUpdate }) => {
  const { income_id, group_sequence } = incomeObject;
      ...
      axios
        ...
        .then((res) => {
          ...
          setFormData({
            ...
            amount: data.amount,
            rcDate: new Date(data.rcDate).toLocaleDateString("en-CA"),
  ...
  const handleDeleteImage = async (fileName) => {
    try {
      await axios.delete(
        `${import.meta.env.VITE_SERVER_URL}/api/income/delete-image`,
        {
          data: { fileName },
        }
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_SERVER_URL}/api/incomes/${incomeId}/${
          income_id.group_sequence
        }`,
        formData
      );
      ...
      if (newImages.length > 0) {
        console.log("if newImages");
        const uploadData = new FormData();
        newImages.forEach((image) => uploadData.append("files", image));
        uploadData.append("income_id", income_id);
        uploadData.append("group_sequence", group_sequence);

        await axios.post(
          `${import.meta.env.VITE_SERVER_URL}/api/income/update-images`,
          uploadData
        );
      }
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-gray-800/60 z-50">
      <div className="bg-white rounded-lg shadow-lg w-2/3 h-2/3 overflow-y-auto p-5">
        <div className="flex justify-between items-center border-b pb-3">
          <h3 className="text-lg font-semibold text-gray-900">แก้ไขรายรับ</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mt-3 space-y-4">
          <div>
            <label className="block text-gray-700">เลขที่บิล</label>
            <input
              type="text"
              name="bill_number"
